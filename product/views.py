...
def crawl_shopee(request):
  ...
  if not auth or auth['permission'] == 'user' or auth[
    'permission'] == 'tiki mod':
...
def training(request):
  auth = request.session.get('auth')
  if not auth or auth['permission'] == 'user':
    raise PermissionDenied()
...
def search(request):
  ...
  if request.POST:
    ...
    if request.session.get('auth'):
      file_name = str(
        request.session.get('auth').get('id')) + '_file'
...
def recent(request):
  ...
  user = User.objects.get(
    id=request.session.get('auth').get('id'))
  products = user.product_set.all().order_by(
    '-productuser__view_date')
...
def destroy_recent(request, id):
  ...
  recent = get_object_or_404(ProductUser, product=id,
                             user=user_id)
  recent.delete()
...
def favorite(request):
  ...
  user = User.objects.get(
    id=request.session.get('auth').get('id'))
  products = user.product_set.filter(
    productuser__favorite=True).order_by(
    '-productuser__view_date')
  paginator = Paginator(products, 8)
  page_number = request.GET.get('page')
  page_obj = paginator.get_page(page_number)
...
def go_to_link(request, id):
  ...
  if product:
    ...
    return HttpResponseRedirect(
      product.product_link.replace(' ', '-').replace('%',
                                                     '-'))
  return HttpResponseRedirect('homepage')


def handle_uploaded_file(f, file_name):
  with open('./static/upload/' + file_name + '.jpg',
            'wb+') as destination:
