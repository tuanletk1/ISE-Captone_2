...
class Shopee:
  ...
  def get_images(self):
    ...
    for product in products:
      ...
      with open(
        './static/images_data/shopee/sp_category_' + str(
          dircount) + '/sp_' + product.original_id.replace(
          'sp_',
          '') + '.jpg',
        'wb') as handler:
  ...
  def crawl(self):
    ...
    for key, category in self.category_list.items():
      new_dir = os.path.join('./static/images_data/shopee/',
                             'sp_category_' + str(dircount))
      if not os.path.isdir(new_dir):
        os.mkdir(new_dir)
      for i in range(0, 25):
        count = count + 1
        if self.params['newest'] == 0:
          self.params['newest'] = self.params[
                                    'newest'] + 60 * i
        else:
        ...
        while True:
          ...
          break
        if i == 0:
          with open('./static/crawl/shopee/log/' + str(
            date.today() + timedelta(days=1)) + "_" +
                    self.params[
                      'match_id'] + ".log", "a+") as file:
            file.write(
              f"{response.json().get('items')[0]['itemid']}\n")
        for record in response.json().get('items'):
          exists = False
          product_id = str(
            record.get('item_basic')['itemid'])
          with open('./static/crawl/shopee/log/' + str(
            date.today()) + "_" + self.params[
                      'match_id'] + ".log",
                    "a+") as file:
            file.seek(0)
          ...
          image = "https://cf.shopee.vn/file/" + str(
            record.get('item_basic')['image'])
          link = "https://shopee.vn/" + \
                 record.get('item_basic')[
                   'name'] + '-i.' + str(
            record.get('shopid')) + '.' + str(
            record.get('item_basic')['itemid'])
          rating = 0
          quantity_sold = 0
          discount = 0
          if record.get('item_basic')['item_rating']:
            rating = str(
              record.get('item_basic')['item_rating'][
                'rating_star'])
          if record.get('item_basic')['sold']:
            quantity_sold = str(
              record.get('item_basic')['sold'])
          if record.get('item_basic')['raw_discount']:
            discount = str(
              record.get('item_basic')['raw_discount'])
          img_data = requests.get(image).content
          with open(
            './static/images_data/shopee/sp_category_' + str(
              dircount) + '/sp_' + product_id + '.jpg',
            'wb') as handler:
            handler.write(img_data)
          file = open('./static/crawl/shopee/sql/' + str(
            date.today() + timedelta(days=1)) + "_" +
                      self.params[
                        'match_id'] + ".sql", "a+",
                      encoding='utf-8')
