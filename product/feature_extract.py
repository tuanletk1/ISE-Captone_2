...
class FeatureExtract:
  ...
  def vector_handle(self, data_folder, model, vectors,
                    paths):
    for image_path in os.listdir(data_folder):
      print(image_path)
      image_full_path = os.path.join(DATA_DIR / data_folder,
                                     image_path)
      image_vector = self.extract_vector(model,
                                         image_full_path)
  ...
  def store(self):
    ...
    for shop in os.listdir(DATA_DIR):
      ...
      for folder in os.listdir(data_folder):
        self.vector_handle(data_folder / folder, model,
                           vectors, paths)
  ...
  def merge_training(self):
    ...
    pickle.dump(paths,
                open('static/search/paths.pkl', 'wb'))
    pickle.dump(vectors,
                open('static/search/vectors.pkl', 'wb'))

  def search(self, search_image):
    model = self.extract_model()
    search_vector = self.extract_vector(model, search_image)
    vectors = pickle.load(
      open("static/search/vectors.pkl", 'rb'))
    paths = pickle.load(
      open("static/search/paths.pkl", 'rb'))
    print(search_vector)
    distance = np.linalg.norm(vectors - search_vector,
                              axis=1)
    samples = 20
    ids = np.argsort(distance)[:samples]
    results = [(paths[id].split('\\')[-1].split('.')[0],
                distance[id]) for id in ids]
