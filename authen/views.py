...
def users(request):
  ...
  if not auth or auth['permission'] != 'admin':
    raise PermissionDenied()
  users = User.objects.filter(
    Q(permission__permission='user'))
...
def mods(request):
  ...
  mods = User.objects.filter(
    Q(permission__permission__contains='mod'))
...
def remove_account(request):
  ...
  if not auth or auth['permission'] != 'admin':
    raise PermissionDenied()
  if request.method == 'POST':
    account = get_object_or_404(User, id=request.POST[
      'remove_id'])
    account.delete()
    return JsonResponse({}, status=200)
  return JsonResponse({}, status=400)
...
def set_session(request, user):
  request.session['auth'] = {
    ...
    'avatar': user.avatar,
    'permission': user.permission_set.all().values().get().get(
      'permission')
  }
